...
import {
  WebView,
  releaseWebView,
  injectJavaScriptWithWebViewKey,
  addOnMessageListenerWithWebViewKey,
...
export default function Portals() {
  ...
  React.useEffect(() => {
    const subscription = addOnMessageListenerWithWebViewKey(
      WEB_VIEW_KEY,
      (eventData) => {
  ...
  const togglePages = () => {
    const nextPage =
      pageNumber === PORTALS_PAGE ? NONPORTALS_PAGE : PORTALS_PAGE;
    setPageNumber(nextPage);
  };

  return (
    <>
      <Button title="Toggle Pages" onPress={togglePages} />
      {pageNumber === PORTALS_PAGE ? (
        <PortalProvider>
          <PortalGatesPage />
        </PortalProvider>
      ) : null}
      {pageNumber === NONPORTALS_PAGE ? <NonPortalsPage /> : null}
    </>
  );
}
...
function PortalGatesPage() {
  ...
  React.useEffect(() => {
    ...
    webViewRef.current.rebind(WEB_VIEW_KEY);
    alert('rebound');
  }, []);
  ...
  return (
    <>
      ...
      <Button title="Teleport to blue gate" onPress={teleportToBlueGate} />
      <Button title="Teleport to green gate" onPress={teleportToGreenGate} />
      <Button title="Release WebView" onPress={release} />
      <View
        style={{
          width: IFRAME_WIDTH,
          height: 160,
      ...
      <View
        style={{ width: IFRAME_WIDTH, height: 200, backgroundColor: 'green' }}
      >
